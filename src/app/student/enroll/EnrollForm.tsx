...
import { Card, CardHeader, CardBody, DatePicker, RadioGroup, Radio, Select, SelectItem, addToast } from '@heroui/react'
import { useRouter } from 'next/navigation'
import React, { FormEvent, useState } from 'react'
import { GiPadlock } from 'react-icons/gi'
import { Sex, Shift } from '@/../generated/prisma'
import { CalendarDate } from '@heroui/system/dist/types'
import { dateValueToDate } from '@/lib/utils'
import { getLocalTimeZone, today } from '@internationalized/date'
import { CldUploadButton, CloudinaryUploadWidgetResults } from 'next-cloudinary'
import { IoImage } from "react-icons/io5";
import { createStudent, deleteImage } from '@/actions/memberActions'
...
export default function EnrollForm() {
  ...
  function nationalChange(result: CloudinaryUploadWidgetResults) {
    if (result.info && typeof result.info === 'object') {
      if (formData.nationalIdCard !== undefined) {
        deleteImage(formData.nationalIdCard)
      }
  ...
  function certificateChange(result: CloudinaryUploadWidgetResults) {
    if (result.info && typeof result.info === 'object') {
      if (formData.certificate !== undefined) {
        deleteImage(formData.certificate)
      }
      const public_id = result.info.secure_url
      setFormData((prev) => ({
        ...prev,
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (validate()) {
      const result = await createStudent(formData as EnrollSchema)
      ...
      <CardHeader className='flex flex-col justify-center items-center' >
        <div className='flex flex-col gap-2 items-center text-secondary'>
          <div className='flex flex-row items-center gap-3'>
            <GiPadlock size={30} />
            <h1 className='text-2xl font-semibold'>Enroll Form</h1>
          </div>
        </div>
      </CardHeader >
            ...
            <div className='flex  gap-2 flex-col'>
              <div className='pb-2'>
                ...
                <div className='flex flex-col gap-4 sm:gap-8 sm:flex-row pb-2'>
                  ...
                  <DatePicker label="Student's date of birth"
                ...
                <RadioGroup
                  label='Sex'
                  defaultValue={Sex.MASCULINE} orientation='horizontal'
                  onValueChange={(e) => {
                    setFormData((prev) => ({
                      ...prev,
                      sex: e as Sex
              ...
              <div className='flex flex-col gap-3'>
                ...
                <div className='flex flex-col gap-4 sm:flex-row sm:gap-8'>
                  ...
                  >
                    <SelectItem key={Shift.DAYTIME}>Morning (6h-13h)</SelectItem>
                    <SelectItem key={Shift.NIGHTTIME}>Afternoon (10h-17h)</SelectItem>
                  </Select>
                </div>
                  ...
                  <div className='flex flex-col gap-4 sm:flex-row sm:gap-8'>
                    <div className='flex-1'>
                      ...
                      >
                        ...
                        <IoImage size={24} />
                        {formData.nationalIdCard === undefined ? "Upload " : "Change "}
                        National ID Card

                      </CldUploadButton>
                      {(wasSubmitted && errors.nationalId) &&
                        <p className='text-sm text-red-500'>A national id card is required</p>
                      }

                    </div>
                    <div className='flex-1'>
                      {(formData.class != 1 || formData.class === undefined) && (<>
                        <CldUploadButton
                          options={{ maxFiles: 1 }}
                          signatureEndpoint={'/api/sign-image'}
                          uploadPreset='nm-testing'
                        ...
                        >
                          ...
                          <IoImage size={24} />
                          {formData.certificate === undefined ? "Upload " : "Change "}
                          Certificate
                        </CldUploadButton>

                        {(wasSubmitted && errors.certificate) &&
                          <p className='text-sm text-red-500'>A certificate is required</p>
                        }
                      </>
              ...
              <Button type='submit' className='mt-4' color='secondary'>Submit</Button>
            </div>
          </div>
        </form>
      </CardBody >
