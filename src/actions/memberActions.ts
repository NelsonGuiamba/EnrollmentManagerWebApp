...
export async function createStudent(data: EnrollSchema): Promise<ActionResult<null>> {
  try {
...
export async function getStudentsByUserId(): Promise<ActionResult<StudentWithEnrollment[]>> {
  try {
    const user = await getAuthUser(Role.MEMBER)
    const userId = user.user.id
...
export async function updateStudent(data: EnrollSchema, id: string): Promise<ActionResult<null>> {
  try {
    const user = await getAuthUser(Role.MEMBER)
    const userId = user.user.id
    const validated = enrollSchema.safeParse(data)
...
export async function createAppeal(data: AppealSchema, enrollmentId: string): Promise<ActionResult<null>> {
  try {
